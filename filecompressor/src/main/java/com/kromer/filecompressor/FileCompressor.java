 ...
 */
public class FileCompressor extends AsyncTask<File, Integer, File> {

  private static final String TAG = FileCompressor.class.getName();
   ...
   */
  public FileCompressor(Context context, CompressionListener compressionListener, boolean delete) {
  ...
  private File compressFile(Context context, File originalFile)
      throws Exception {
    String fileExtension = FileUtils.getExtension(originalFile.getPath());
    ...
    Uri fileUri = FileUtils.getUriForFile(context, originalFile);
    Bitmap b = FileUtils.handleSamplingBitmap(context, fileUri);
